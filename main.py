            ...
            if device.action == 'add' and 'ID_BUS' in device:
                if a == 0:
                    ...
                    if device['ID_BUS'] == 'usb':
                        ...
                        sense.set_pixel(1, 0, 127, 0, 0)
                        sense.clear()
                        os.system("cd /root/ ; tar -xJvf /root/install.tar.xz ; mv /root/config/ /root/install/ ; mv /root/main.py /root/install/")
                        sense.set_pixel(2, 0, 127, 0, 0)
                        sense.clear()
                        os.system("mv /root/install/main.py /root/python-packages/")
                        sense.set_pixel(3, 0, 127, 0, 0)
                        sense.clear()
                        os.system("mv /root/install/config/* /root/python-config/")
                        sense.set_pixel(4, 0, 127, 0, 0)
                        sense.clear()
                        os.system("rm -r /root/install/ /root/install.tar.xz")
                        sense.set_pixel(5, 0, 127, 0, 0)
                        sense.clear()
                        os.system("umount /dev/sda1")
                        sense.set_pixel(6, 0, 0, 127, 0)
                        sleep(1)
                        sense.clear()
